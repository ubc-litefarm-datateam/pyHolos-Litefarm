...
class EmissionCalculator:
    """
    A calculator for deriving nitrogen-based emissions and their equivalent CO2 impact 
    from crop nitrogen residue and emission factor.

    Parameters
    ----------
    ef_data : dict
        Input data containing emission factors.
    n_data : dict
        Input data containing nitrogen values.

    Attributes
    ----------
    ef_data : dict
        Validated input data containing emission factors.
    n_data : dict
        Validated input data containing crop nitrogen residue.
    EF : float
        Emission factor from validated input data.
    n_crop_residue : float
        Crop nitrogen residue (CRN) value from validated input data.

    Methods
    -------
    validate_input(ef_data, n_data)
        Validates the input data to ensure all required fields are correctly formatted.
    calculate_n_crn_direct()
        Calculates the direct nitrogen emission from crop residue (n_crn_direct).
    calculate_n_other_direct()
        Sets the values for other sources of direct nitrogen emissions.
    calculate_n_crop_direct()
        Calculates the total direct nitrogen emission from all crop sources (n_crop_direct).
    convert_n_crop_direct_to_n2o()
        Converts the total direct nitrogen emission from crops to N2O emissions.
    calculate_n2o_crop_direct_to_co2e()
    ...
    ------
    ValueError
        If essential keys are missing in the input data.
    TypeError
        If the values under emission factor data or nitrogen data are not of 
        type int or float.
    """
    ...
    def validate_input(self, ef_data, n_data):
        ...
        ----------
        ef_data : dict
            The emission factor data to validate.
        n_data : dict
            The nitrogen data to validate.

        Raises
        ------
        ValueError
            If required keys are missing from the emission factor data or nitrogen data.
        TypeError
            If the values under emission factor data or nitrogen data are not of type int or float.
        """
        required_ef_key = ["EF"]
    ...
    def calculate_n_crn_direct(self):
        """
        Calculates the direct nitrogen emission from crop residue (n_crn_direct).
        Equation 2.6.5-2 in Holos version 4.0 technical report.

        Returns
        -------
        float
            The direct nitrogen emission from crop residue.
        """
    ...
    def calculate_n_other_direct(self):
        """
        Sets the values for other sources of direct nitrogen emissions, currently set to 0. 
        Can be modified to include other sources.
        """
        self.n_sn_direct = 0
    ...
    def calculate_n_crop_direct(self):
        """
        Calculates the total direct nitrogen emission from all crop sources (n_crop_direct).
        Equation 2.6.9-1 in Holos version 4.0 technical report.

        Returns
        -------
        float
            The total direct nitrogen emission from crop residue.
        """
        if (
            not hasattr(self, "n_sn_direct")
            or not hasattr(self, "n_crnmin_direct")
            or not hasattr(self, "n_on_direct")
        ):
    ...
    def convert_n_crop_direct_to_n2o(self):
        ...
        -------
        float
            The N2O emissions derived from direct nitrogen emissions.
        """
    ...
    def calculate_n2o_crop_direct_to_co2e(self):
        ...
        -------
        float 
            The CO2 equivalent of N2O emissions.
        """
    ...
    def get_emission(self):
        ...
        -------
        dict
            A dictionary containing all calculated emissions including nitrogen, 
            N2O, and CO2 equivalent values.
        """
...
if __name__ == "__main__":
    ...
    test_farm_n = {"n_crop_residue": 560.2468642105264}
    test_farm_ef = {
