...
class EmissionAggregator:
    """
    A class designed to perform sensitivity analysis on emissions influenced by both emission factors and nitrogen data.
    
    Attributes
    ----------
    ef_data (dict): Contains emission factor data for different variables.
    n_data (dict): Contains nitrogen  data for different variables.
    variables (list): List of variable derived from ef_data and n_data for conducting sensitivity analysis..
    mode (str): Operation mode which can be 'farmer' for simplified outputs or 'scientific' for detailed analysis.
    results (dict): Stores the detailed results of the analysis for each variable.
    output (dict): Stores the final output depending on the operation mode.
    """
    def __init__(self, ef_data, n_data, operation_mode = 'farmer'):
        """
        Initializes SensitivityEmission with emission factor data, nitrogen data, and an operation mode.
        
        Args:
            ef_data (dict): The emission factor data.
            n_data (dict): The nitrogen-related data.
            operation_mode (str, optional): Mode of operation, defaults to 'farmer'. Can be 'farmer' or 'scientific'.
        """
    ...
    def perform_analysis(self):
        ...
        Returns:
            dict: The computed results for each variable, structured by different types of emission outcomes.
        """
    ...
    def get_result(self):
        """
        Returns the analyzed results according to the specified mode. If mode is 'farmer', only returns unnested results.
        
        Returns:
            dict: The result of the analysis based on the specified mode.
        """
        output_temp = self.perform_analysis()
        if self.mode == 'farmer':
            self.output = output_temp['EF'] # All dictionary values are the same, can select other keys
        elif self.mode == 'scientific':
    ...
    def prepare_ef_input_for_ec(self, selected_variable, value):
        """
        Prepares the input for an emission calculation by adjusting the emission factor for a selected variable.
        
        Args:
            selected_variable (str): The variable to adjust in the emission factor data.
            value (float): The new value to set for the emission factor.
        
        Returns:
            dict: The prepared emission factor data for the selected variable.
        """
        if self.mode == 'farmer':
    ...
    def prepare_n_input_for_ec(self, selected_variable, value):
        ...
        Args:
            selected_variable (str): The variable to adjust in the nitrogen data.
            value (float): The new value to set for the nitrogen parameter.
        
        Returns:
            dict: The prepared nitrogen data for the selected variable.
        """
        if self.mode == 'farmer':
...
if __name__ == "__main__":
    ...
    sci_emission_calc = EmissionAggregator(ef_data_scientific, n_data_scientific, operation_mode = 'scientific')
    output = sci_emission_calc.get_result()
    print(output)
    print("-"*50)
    print("\n"*2)