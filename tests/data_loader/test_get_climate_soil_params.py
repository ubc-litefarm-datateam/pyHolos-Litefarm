...
def test_eco_id_added(manager_default, farm_data):
    ...
    assert manager_default.eco_id == 540, "eco_id is incorrect"
...
def test_farmer_mode_outputs(manager_default, manager_external_farmer):
    ...
    assert round(result_default["P"][0], 2) == 652
    assert round(result_default["PE"][0], 2) == 556
    assert round(result_default["soil_texture"][0], 2) == 0.49

    assert round(result_external["P"][0], 2) == 403.53
    assert round(result_external["PE"][0], 2) == 493.59
...
def test_scientific_mode_lengths(manager_external_scientific):
    ...
    assert result_scientific["P"][0] == 403.53
    assert result_scientific["PE"][0] == 493.59
    assert result_scientific["FR_Topo"][0] == 11.71
    assert result_scientific["locations"][0][0] == -71.5189528
    assert result_scientific["locations"][0][1] == 46.4761852
