...
def test_data():
    return {
        "farm_data": {"area": 10, "yield": 5000, "group": "root"},
        "crop_group_params": {
...
def setup(test_data):
    ...
    expected_c_p = (
        (
            test_data["farm_data"]["yield"]
            + test_data["farm_data"]["yield"]
            * test_data["crop_group_params"]["S_p"]
            / 100
        )
        * (1 - test_data["crop_parameters"]["moisture"] / 100)
        * test_data["crop_group_params"]["carbon_concentration"]
    )
    expected_c_p_to_soil = expected_c_p * test_data["crop_group_params"]["S_p"] / 100
    expected_c_s = (
        expected_c_p
        * (test_data["crop_parameters"]["R_s"] / test_data["crop_parameters"]["R_p"])
        * (test_data["crop_group_params"]["S_s"] / 100)
    )
    expected_c_r = (
        expected_c_p
        * (test_data["crop_parameters"]["R_r"] / test_data["crop_parameters"]["R_p"])
        * (test_data["crop_group_params"]["S_r"] / 100)
    )
    expected_c_e = expected_c_p * (
        test_data["crop_parameters"]["R_e"] / test_data["crop_parameters"]["R_p"]
    )
    expected_grain_n = (expected_c_p_to_soil / 0.45) * (
        test_data["crop_parameters"]["N_p"] / 1000
    )
    expected_straw_n = (expected_c_s / 0.45) * (
        test_data["crop_parameters"]["N_s"] / 1000
    )
    expected_root_n = (expected_c_r / 0.45) * (
        test_data["crop_parameters"]["N_r"] / 1000
    )
    expected_exudate_n = (expected_c_e / 0.45) * (
        test_data["crop_parameters"]["N_e"] / 1000
    )
    expected_above_ground_residue_n = expected_straw_n
    expected_below_ground_residue_n = expected_grain_n + expected_exudate_n
    expected_n_crop_residue = (
        expected_above_ground_residue_n + expected_below_ground_residue_n
    ) * test_data["farm_data"]["area"]
...
def test_above_ground_residue_n(setup):
    ...
    assert (
        pytest.approx(above_ground_residue_n, 0.1)
        == setup["expected_above_ground_residue_n"]
    )
...
def test_below_ground_residue_n(setup):
    ...
    assert (
        pytest.approx(below_ground_residue_n, 0.1)
        == setup["expected_below_ground_residue_n"]
    )
...
def test_above_ground_carbon_input(setup):
    ...
    assert (
        pytest.approx(above_ground_carbon_input, 0.1)
        == setup["expected_above_ground_carbon_input"]
    )
...
def test_below_ground_carbon_input(setup):
    ...
    assert (
        pytest.approx(below_ground_carbon_input, 0.1)
        == setup["expected_below_ground_carbon_input"]
    )
...
def test_group_handling_annual(test_data):
    ...
    assert (
        pytest.approx(calculator.above_ground_residue_n(), 0.2) == expected_above_ground
    )
    assert (
        pytest.approx(calculator.below_ground_residue_n(), 0.2) == expected_below_ground
    )
...
def test_group_handling_perennial(test_data):
    ...
    calculator = CropResidueCalculator(data)
    expected_above_ground = calculator.grain_n() + calculator.straw_n()
    expected_below_ground = (
        calculator.root_n() * (data["crop_group_params"]["S_r"] / 100)
        + calculator.exudate_n()
    )
    assert (
        pytest.approx(calculator.above_ground_residue_n(), 0.2) == expected_above_ground
    )
